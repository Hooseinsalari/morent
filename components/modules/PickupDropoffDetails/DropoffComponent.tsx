...
// types
import { DropOffComponentProps, MinTimeType } from "@/types";
...
// helper
import { formatDate, formatTime, getMonthName } from "@/helper/functions";
...
const DropoffComponent = ({ dropOff, setDropOff }: DropOffComponentProps) => {
  ...
  const listBoxHandler = (e: any) => {
    ...
    setDropOff({ ...dropOff, location: cityData?.name! });
    ...
    <div className="bg-white p-4 w-full lg:w-1/2 rounded-lg lg:flex-grow">
      ...
      <div className="flex items-center justify-between mt-4">
              ...
              {citiesInIran.map((c) => (
                <ListBoxItem
                  ...
                  className={`my-4 p-2 hover:bg-[#e5e7eb] cursor-pointer ${
                    c.name === dropOff.location ? "bg-[#e5e7eb]" : ""
                  }`}
                >
                  {c.name}
                </ListBoxItem>
        ...
        <div className="md:ml-5 cursor-pointer relative z-30">
          ...
          >
            <span>
              {dropOff.date.day && dropOff.date.month
                ? `${dropOff.date.day} ${getMonthName(dropOff.date.month)}`
                : "select..."}
            </span>
            <Image
              src={Arrow}
